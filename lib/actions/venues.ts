...
export async function createVenueAction(orgSlug: string, orgId: string, _prev: ActionState, formData: FormData): Promise<ActionState> {
  ...
  const venue = await prisma.venue.create({
    data: {
      ...
      onlineUrl: parsed.data.isOnline ? parsed.data.onlineUrl || null : null,
      addressLine1: parsed.data.isOnline ? null : parsed.data.addressLine1 || null,
      addressLine2: parsed.data.isOnline ? null : parsed.data.addressLine2 || null,
      city: parsed.data.isOnline ? null : parsed.data.city || null,
      state: parsed.data.isOnline ? null : parsed.data.state || null,
      postalCode: parsed.data.isOnline ? null : parsed.data.postalCode || null,
